...
Redistribution must preserve these notices and the accompanying license texts. Microsoft OpenMP is not redistributed in SCALEGO packages: users install the official runtime separately. The development installer may obtain the DLL inside the original upstream archive for local use; the package filter excludes it. See [Microsoft redistribution terms](https://learn.microsoft.com/en-us/visualstudio/releases/2022/redistribution) for that component. SCALEGO's own source has not been assigned an open-source license; third-party licenses do not license the application's original code.
