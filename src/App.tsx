...
export default function App() {
  ...
  const [theme, setTheme] = useState<'dark' | 'light'>(() => localStorage.getItem('scalego-theme') === 'light' ? 'light' : 'dark')
  ...
  const cannotProcess = !desktop || !selected.length || importing || modelUnavailable
  const resultWarning = stale || Boolean(job?.result?.warnings.length)
  ...
  return <div className="app" onDragOver={event => { event.preventDefault(); if (desktop && !state.busy) setDragging(true) }} onDragLeave={event => { if (!event.currentTarget.contains(event.relatedTarget as Node)) setDragging(false) }} onDrop={event => { event.preventDefault(); setDragging(false); if (!desktop || state.busy) return; const files = Array.from(event.dataTransfer.files); void action(async () => { setImporting(true); try { const result = await window.scalego!.drop(files); if (result.errors.length) setNotice({ text: result.errors.join('\n'), error: true }) } finally { setImporting(false) } }) }}>
    ...
    <div className="notification-stack">
      {notice && <div className={`notice ${notice.error ? 'error' : ''}`} role={notice.error ? 'alert' : 'status'}>
        {notice.error ? <AlertCircle size={17}/> : <Check size={17}/>}<span>{notice.text}</span>
        <button className="icon-button" aria-label="Закрыть сообщение" onClick={() => setNotice(undefined)}><X size={16}/></button>
      </div>}
      {latest?.error && currentError !== dismissedError && <div className="job-message error" role="alert">
        <AlertCircle size={16}/><span>{latest.error}</span>
        <button className="icon-button" aria-label="Закрыть ошибку обработки" onClick={() => setDismissedError(currentError)}><X size={16}/></button>
      </div>}
    </div>
    <footer className="action-bar">
      ...
      <div className="run-actions">
        {state.jobs.some(j => j.status === 'interrupted') && !state.busy && <button className="button" onClick={() => action(() => window.scalego!.resume())}>Продолжить очередь</button>}
        {done.length > 1 && !state.busy && <button className="button" disabled={saving} onClick={() => action(async () => { setSaving(true); try { const result = await window.scalego!.exportAll(); setNotice({ text: 'Сохранено файлов: ' + result.count, error: false }) } finally { setSaving(false) } })}><Download size={16}/>Сохранить все</button>}
        {job?.result && !state.busy && <button className="button" onClick={() => action(async () => { const saved = await window.scalego!.export(job.id); if (!saved.canceled) setNotice({ text: `Сохранено: ${saved.name}`, error: false }) })}><Download size={16}/>Сохранить</button>}
        {state.outputDirectory && <button className="icon-button" aria-label="Открыть папку результатов" onClick={() => action(() => window.scalego!.reveal())}><FolderOpen size={18}/></button>}
        {state.busy ? <button className="button" onClick={() => action(() => window.scalego!.cancel())}><Square size={14}/>Отменить</button>
          : <span className="metal-action">
            <button className="button primary" disabled={cannotProcess} onClick={() => action(() => window.scalego!.start(selected, options))}><Play size={15}/>{selected.length > 1 ? `Обработать ${selected.length}` : 'Обработать'}</button>
            <MetalAccent paused={state.busy || cannotProcess} theme={theme}/>
          </span>}
      </div>
    </footer>
