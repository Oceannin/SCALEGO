...
export default function App() {
  ...
  const [engine, setEngine] = useState<Engine>({ available: false, name: 'Real-ESRGAN', message: 'Проверка движка…' })
  ...
  useEffect(() => {
    ...
    bridge.engine().then(value => { if (live) { setEngine(value); if (!value.available) setOptions(current => ({ ...current, method: 'lanczos' })) } }).catch(error => setNotice({ text: error.message, error: true }))
